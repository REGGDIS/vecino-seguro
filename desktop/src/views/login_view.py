...
class LoginView(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        izquierda = QFrame()
        izquierda.setStyleSheet("background-color: #0F3866;")
        ...
        titulo.setStyleSheet("color: #0F3866; font-size: 28px; font-weight: 700;")
        der_layout.addWidget(titulo)

        subtitulo = QLabel("Ingresa con tu RUT y contraseña")
        subtitulo.setStyleSheet("color: #6B7280; font-size: 14px;")
        ...
        demo_card.setStyleSheet(
            "background-color: #F5F7FA; border: 1px dashed #DDE3EC; border-radius: 8px;"
        )
    ...
    def _mk_demo_titulo(self, texto: str) -> QLabel:
        l = QLabel(texto)
        l.setStyleSheet(
            "color: #4A5568; font-weight: 700; font-size: 11px; "
    ...
    def _mk_demo_linea(self, rol: str, rut: str, password: str) -> QLabel:
        l = QLabel(
            f"<span style='color:#1B4F8C; font-weight:600;'>{rol}</span> "
            f"<span style='color:#1A202C;'>{rut}</span> "
            f"<span style='color:#6B7280;'>·</span> "
            f"<span style='color:#1A202C;'>{password}</span>"
        )
    ...
    def _on_rut_changed(self, texto: str) -> None:
        ...
        if RutValidator.es_valido(texto):
            self.lbl_rut_status.setStyleSheet("color: #2C8C46; font-size: 11px;")
