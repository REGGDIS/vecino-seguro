...
class EmergencyListView(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        self.lbl_total.setStyleSheet("color: #6B7280; font-size: 12px;")
        ...
        msg.setStyleSheet("color: #6B7280; font-size: 13px; background: transparent;")
    ...
    def _build_detalle_panel(self) -> QFrame:
        ...
        self.lbl_id.setStyleSheet("color: #6B7280; font-size: 12px; font-weight: 600;")
        ...
        self.lbl_tipo.setStyleSheet("font-size: 20px; font-weight: 700; color: #0F3866;")
        ...
        self.lbl_descripcion.setStyleSheet(
            "background-color: #F5F7FA; padding: 12px; border-radius: 6px; "
    ...
    def _mk_field(self, etiqueta: str, valor: str) -> QFrame:
        ...
        l2 = QLabel(valor)
        l2.setStyleSheet("color: #1A202C; font-size: 13px;")
        l2.setWordWrap(True)
        wlay.addWidget(l1)
        wlay.addWidget(l2)
    ...
    def refrescar(self) -> None:
        ...
        for row, em in enumerate(emergencias):
            self._set_cell(row, 0, f"#{em.id}", align=Qt.AlignCenter,
                           color="#6B7280", weight=600, data=em.id)
            ...
            self._set_cell(row, 5, em.fecha_reporte.strftime("%d-%m-%Y %H:%M"),
                           align=Qt.AlignCenter, color="#6B7280")
    ...
    def _color_urg(self, v: str) -> str:
        return {"Crítica": "#9B2C2C", "Alta": "#C05621",
                "Media": "#975A16", "Baja": "#22543D"}.get(v, "#1A202C")

    def _color_est(self, v: str) -> str:
        return {"Pendiente": "#DD6B20", "En revisión": "#1B4F8C",
                "Atendido": "#2C8C46", "Resuelto": "#4A5568"}.get(v, "#1A202C")
    ...
    def _mostrar_detalle(self, emergencia_id: int) -> None:
        ...
        # Badge de estado (estilo inline)
        badge_estado_styles = {
            "Pendiente":   ("#FFF4E5", "#DD6B20"),
            "En revisión": ("#E5F0FB", "#1B4F8C"),
            "Atendido":    ("#E5F4EA", "#2C8C46"),
            "Resuelto":    ("#E2E8F0", "#4A5568"),
        }
        bg, fg = badge_estado_styles[em.estado.value]
        self.lbl_estado_badge.setText(em.estado.value)
        self.lbl_estado_badge.setStyleSheet(
            f"background-color: {bg}; color: {fg}; "
            f"border: none; border-radius: 10px; "
            f"padding: 4px 12px; font-weight: 600; font-size: 11px;"
        )

        # Badge de urgencia (estilo inline)
        badge_urg_styles = {
            "Crítica": ("#FED7D7", "#9B2C2C"),
            "Alta":    ("#FEEBC8", "#C05621"),
            "Media":   ("#FEFCBF", "#975A16"),
            "Baja":    ("#C6F6D5", "#22543D"),
        }
        bgu, fgu = badge_urg_styles.get(em.nivel_urgencia.value, ("#E2E8F0", "#4A5568"))
        self.lbl_urg_badge.setText("● " + em.nivel_urgencia.value)
        self.lbl_urg_badge.setStyleSheet(
            f"background-color: {bgu}; color: {fgu}; "
            f"border: none; border-radius: 10px; "
